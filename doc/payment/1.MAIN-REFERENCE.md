...
- **Stripe Integration**: Real money deposits via Stripe Checkout and payouts via Stripe Connect
- **Wallet-Based Tracking**: Internal wallet system tracks all transactions in database
...
- **Automatic Refunds**: Failed transactions and cancellations trigger instant wallet adjustments
- **Real-Time Updates**: WebSocket integration for instant notifications
- **Transaction History**: Complete audit trail of all financial activities
- **Admin-Approved Payouts**: Contractors request withdrawals, admin approves via Stripe Connect
...
**Wallet Management**
- Handles user balances (tracked in database)
- Processes deposits via Stripe Checkout
...
**Stripe Integration**
- Stripe Checkout for customer deposits (hosted payment page)
- Stripe Connect for contractor payouts (admin-initiated transfers)
...
- Wallet-based balance tracking (no real-time Stripe balance sync)
...
- Approves contractor withdrawal requests
...
- Minimum deposit: $10
- Integrated with Stripe Checkout (hosted payment page)
- Backend generates Stripe Checkout Session URL
- Customer opens URL in default browser (not in-app)
- Webhook confirms payment and updates wallet balance
...
- Wallet balances updated immediately (no real money transfer)
- Customer wallet decreased by total charge
- Admin wallet increased by total charge
...
- Wallet balances reversed (full refund)
- Application status reset
