...
JOB COMPLETED
  ↓
Customer marks complete → Admin reviews
  ↓
...
|------|------|------------|------------|---------------|
| `deposit` | Customer deposits | Customer wallet +$ | Stripe → Platform | Customer |
...
| `contractor_payout` | Job completion | Admin -$, Contractor +$ | Platform → Contractor | Admin |
| `refund` | Cancel/Reject | Admin -$, Customer +$ | None | System |
| `withdrawal` | Contractor withdraws | Contractor -$ | Platform → Contractor | Admin |
...
- ✅ Both customers and contractors can have `stripeCustomerId`
- ✅ Only contractors have `stripeConnectAccountId`
...
```typescript
// ✅ Good - always create transaction record
await TransactionModel.create({
  type: 'wallet_transfer',
  amount,
  from: senderId,
