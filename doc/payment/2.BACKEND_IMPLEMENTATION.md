...
**Status**: ✅ Stripe Integration Complete (Phases 1-6)
...
│   ├── wallet/
│   │   ├── services/
│   │   │   ├── deposit.service.ts          # Stripe Payment Intents
│   │   │   ├── withdraw.service.ts         # Stripe Transfers
...
│   └── webhooks/
│       ├── services/
│       │   └── stripe-webhook.service.ts   # Webhook handler
│       └── webhook.route.ts
├── db/models/
│   ├── user.model.ts                       # Stripe customer/account IDs
│   ├── wallet.model.ts                     # Pending deposits
│   └── transaction.model.ts                # Stripe IDs, idempotency
├── lib/
│   └── stripe.ts                           # Stripe client initialization
├── jobs/
│   └── retry-failed-transactions.ts        # Auto-retry failed txns
└── middleware/
...
Customer Deposit:
┌─────────────────────────────────────────────────────────┐
│  1. Frontend creates Payment Method (flutter_stripe)     │
│  2. Backend creates Payment Intent (with confirmation)   │
...
┌─────────────────────────────────────────────────────────┐
│  1. Contractor has verified Connect account              │
│  2. Backend creates Stripe Transfer                      │
│  3. Wallet balance deducted atomically                   │
...
|------|-------------|------|
| `deposit` | Wallet deposit via Stripe | Customer → Wallet |
| `withdrawal` | Withdrawal via Stripe Transfer | Wallet → Bank |
| `escrow_hold` | Money held for offer | Wallet → Escrow |
| `escrow_release` | Money released from escrow | Escrow → Wallet |
| `platform_fee` | 5% fee on offer accept | Escrow → Admin |
| `service_fee` | 20% fee on job complete | Escrow → Admin |
| `contractor_payout` | 80% to contractor | Escrow → Contractor |
| `refund` | Refund on reject/cancel | Escrow → Customer |
...
async function handlePaymentIntentSucceeded(
  ...
  if (!transaction) {
    console.log(`⚠️ Transaction not found for ${paymentIntent.id}`);
    return;
  }
...
Before Complete:
├── Customer escrow: $100
├── Admin balance: $5 (platform fee from accept)
└── Contractor balance: $0

After Complete:
├── Customer escrow: $0
├── Admin balance: $25 (+$20 service fee)
└── Contractor balance: $80 (payout)
```
...
- ✅ Failed payments don't update balance
- ✅ Pending deposits decrease after webhook
- ✅ Transaction records created correctly
- ✅ Stripe customer ID saved to user
- ✅ Webhook signature verification working
- ✅ Rate limiting enforced
- ✅ Idempotency prevents duplicates
- ✅ Connect onboarding flow works
- ✅ Withdrawals transfer to connected account
- ✅ Transfer reversal on wallet update failure
