...
export const sendInvite: RequestHandler = async (req, res) => {
	try {
		...
		if (job.customerId.toString() !== customerId) {
			return sendError(res, 403, "You can only invite contractors to your own jobs");
		}
		...
		// Check if contractor has already applied to this job
		...
		if (existingApplication) {
			return sendError(
				res,
				400,
				"This contractor has already applied to this job",
			);
		}
		...
		await NotificationService.sendToUser({
			...
			body: `${customer?.full_name || "A customer"} has invited you to work on "${job.title}"`,
			type: "job_posted",
			data: {
				jobId: jobId,
