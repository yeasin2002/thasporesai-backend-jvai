 ...
 *
 * NEW PAYMENT SYSTEM (v2.0):
 * This endpoint creates a CompletionRequest for admin approval.
 * Payment is released only after admin approves the completion.
 *
 * Flow:
 * 1. Customer marks job complete → Creates CompletionRequest (status: "pending")
 * 2. Admin reviews and approves → Payment released to contractor
 *
 * See: src/api/job/services/complete-job.service.ts for the main implementation
 */
export const markAsComplete: RequestHandler = async (req, res) => {
  try {
    ...
    if (!job) {
      return sendNotFound(res, "Job not found or you are not the owner");
    }
    ...
    if (!offer) {
      return sendNotFound(res, "No accepted offer found for this job");
    }
    ...
    // 6. Create completion request for admin approval
    ...
    // 7. Send notification to admin (optional)
    ...
    return sendCreated(res, "Completion request created successfully", {
      ...
      status: "pending",
      message:
        "Your completion request has been submitted. Admin will review and approve the payment release.",
      payment: {
        contractorWillReceive: offer.contractorPayout,
