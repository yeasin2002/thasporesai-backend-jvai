...
> = async (req, res) => {
	try {
		...
		// 8. Create offer (linked to invite instead of application)
		...
		// 11. Update invite status to indicate offer sent
		invite.status = "offer_sent" as any; // You may need to add this status to JobInvite model
		await invite.save();
